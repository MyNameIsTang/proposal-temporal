...
export class PlainDateTime {
  ...
  with(temporalDateTimeLike, options = undefined) {
    ...
    const date = ES.DateFromFields(calendar, fields, GetIntrinsic('%Temporal.PlainDate%'), overflow);
    let year = GetSlot(date, ISO_YEAR);
    let month = GetSlot(date, ISO_MONTH);
    let day = GetSlot(date, ISO_DAY);
    const time = ES.TimeFromFields(calendar, fields, GetIntrinsic('%Temporal.PlainTime%'), overflow);
